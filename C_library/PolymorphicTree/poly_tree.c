...
int static recursive_check_subtree(node_t *node, int (*cmp)(void*, void*)) {
    if (!node) return TRUE;
...
/*static node_t *recursive_delete_key(void *key, node_t *node, int dir,
                                                int (*cmp)(void*, void*)) {
    if ((dir > 0 ? node->right : node->left) == NULL) return NULL;
    int result = cmp((dir > 0 ? node->right->data : node->left->data), key);
    node_t *rem_node = NULL;
    if (result == 0) {
        rem_node = (dir > 0 ? node->right->left : node->left->right);
        (dir > 0 ? node->right : node->left) = 
                            (dir > 0 ? node->right->right : node->left->left);
        return rem_node;
    } else return recursive_delete_key(key,
                                       (dir > 0 ? node->right : node->left),
                                                                result, cmp);
}*/
static node_t *recursive_delete_key(void *key, node_t *node, int dir,
